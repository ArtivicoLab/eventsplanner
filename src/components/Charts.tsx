...
/** Vertical column trend, CSS columns, JS-normalized. Pass `min`/`max` for a
    fixed domain — omit to auto-scale to the data's own range. */
        ...
        return (
          <div key={i} className="chart-column">
            <div
              ...
              style={{ height: Math.max(2, h), background: color, opacity: 0.55 + 0.45 * ((p.value - lo) / span) }}
            >
              {showValues && p.value > 0 && <span className="chart-column__value">{p.value}</span>}
            </div>
            {points.length <= 8 && <span className="muted chart-column__label">{p.label}</span>}
          </div>
        );
      })}
    </div>
