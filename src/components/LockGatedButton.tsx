...
// one flanking each side of the button — both must be unlocked before a tap
// does anything. Not real security, just deliberate friction so a scary
// action ("erase everything") isn't a single misplaced tap. Tapping the
// button while still locked shakes it and gives a haptic buzz instead of
// silently doing nothing. Unlocking a latch bursts a little CSS-only confetti.
//
// While still locked, each latch's background strobes red/blue like a police
// light — driven by JS setTimeout with a RANDOMIZED delay each tick (not a
// CSS @keyframes loop), so the flash sequence never settles into an exact
// repeating rhythm. Stops the instant that latch opens.
...
// same in light/dark mode, like a real siren.
...
function Lock({ open, label, onToggle }: { open: boolean; label: string; onToggle: () => void }) {
  ...
  return (
    <span className="lockgate__lockwrap">
      <button
        type="button"
        className={["lockgate__lock", open ? "lockgate__lock--open" : `lockgate__lock--flash${flashColor}`].join(" ")}
        aria-pressed={open}
        aria-label={label}
        onClick={handleClick}
      >
      ...
      {confetti.map((c) => (
        <span
          ...
          style={{ "--dx": `${c.dx}px`, "--dy": `${c.dy}px`, "--rot": `${c.rotate}deg`, background: c.color } as CSSProperties}
        />
      ))}
    </span>
  );
...
export function LockGatedButton({
  ...
  return (
    <div className="lockgate">
      <Lock open={lock1Open} label={lock1Open ? "Latch 1 unlocked" : "Unlock latch 1"} onToggle={() => setLock1Open((v) => !v)} />
      <button
        type="button"
        className={[
          "btn",
      ...
      <Lock open={lock2Open} label={lock2Open ? "Latch 2 unlocked" : "Unlock latch 2"} onToggle={() => setLock2Open((v) => !v)} />
    </div>
  );
}
