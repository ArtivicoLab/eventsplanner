...
interface Props {
  ...
  /** Empty/neutral state: a muted dotted ring, no fill. */
  dotted?: boolean;
  ariaLabel?: string;
}
...
export function ProgressRing({
  ...
  const defaultLabel = dotted
    ? label ? `${label}: no data yet` : "No data yet"
    : label ? `${label}: ${pct}%` : `${pct}%`;
    ...
    >
      ...
      >
        {center ??
          (showPct && (
            <>
              <span style={{ fontSize: size * 0.26, fontWeight: 800, letterSpacing: "-0.02em", fontFamily: "var(--font-display)" }}>
                {Math.round(anim * 100)}%
              </span>
              {label && <span style={{ fontSize: 11, color: "var(--muted)", fontWeight: 600 }}>{label}</span>}
            </>
          ))}
      </div>
    </div>
