// ALL date math for the app goes through this module.
// Dates are stored as plain ISO calendar dates (yyyy-mm-dd), no time component
// (times are a separate free-form "HH:mm" field, on Events and on Guests).
...
/** A raw "HH:mm" field (Event start/end time, Guest arrival time) rendered
    for display as "3:00 PM" instead of the native 24-hour input value.
    Returns "" unchanged for an unset time rather than throwing. */
...
/** True when [aStart,aEnd] and [bStart,bEnd] (inclusive ISO ranges) overlap at all. */
