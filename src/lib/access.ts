// Etsy purchase-code gate. Static app, no backend — so this is a soft,
// client-side check against a list baked in at build time, not real license
// enforcement. It exists to keep casual visitors on the demo data and point
// genuine buyers at the real (blank, Sheets-connected) experience.
//
...
// and isn't fixable without a server to own the rate limit). tryUnlock()
// below is an honest, not bulletproof, speed bump: an escalating lockout
// after repeated wrong guesses through the real UI, persisted to BOTH
...
const LS_THROTTLE = "ep.unlockThrottle";
